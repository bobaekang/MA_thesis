...
## and construct functions to add muldi-modal variables to the Divvy trip data                  
## 2-5. adding binary and non-binary multi-model variables to Divvy trip data
...
# (using lubiridate::fast_strptime; still too slow)
# for trips that start in a potentially multi-modal manner
multimodalFrom <- function(from_input, departures_input){ 
  ...
  dep <- fast_strptime(str_c(data$startdate, data$departure_time, sep = " "),
                       format = "%Y-%m-%d %H:%M:%OS",
                       tz = "America/Chicago")
  data$close <- (5 >= abs(difftime(dep, data$starttime,
                                   tz = "America/Chicago",
                                   units = c("mins"))) |
                   1< abs(difftime(dep, data$starttime,
                                   tz = "America/Chicago",
                                   units = c("mins"))))*1
...
# for trips that end in a potentially multi-modal manner
multimodalTo <- function(to_input, arrivals_input){ 
  ...
  arr <- fast_strptime(str_c(data$stopdate, data$arrival_time, sep = " "),
                       format = "%Y-%m-%d %H:%M:%OS",
                       tz = "America/Chicago")
  data$close <- (5 >= abs(difftime(data$stoptime, arr,
                                   tz = "America/Chicago",
                                   units = c("mins"))) |
                   1 < abs(difftime(data$stoptime, arr,
                                    tz = "America/Chicago",
                                    units = c("mins"))))*1
...
# Functions to add multimode and multimode_n
# (faster version using basic R functions)
multimodalFrom1 <- function(from_input, departures_input){
...
multimodalTo1 <- function(to_input, arrivals_input){
  # join divvy trips and cta routes data (it gets very large!)
  data <- left_join(to_input, arrivals_input)
